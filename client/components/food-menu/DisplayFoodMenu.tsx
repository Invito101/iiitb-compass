...
export default function DisplayFoodMenu({
  ...
  const getBadgeText = (dt: string) => {
    switch (dt.toLowerCase()) {
      ...
      case "non-veg":
      case "nonveg": return "text-red-200";
                      ...
                      <div key={entry.id} className="flex items-center justify-between px-4 py-3 border-b last:border-b-0">
                        <div className="flex flex-col">
                          <div className="flex items-center space-x-2">
                            ...
                            {entry.foodItem.dietType.map(dt => (
                              <Badge
                                key={dt}
                                className={`${getBadgeBg(dt)} ${getBadgeText(dt)}`}
                              >
                                {dt}
                              </Badge>
                            ))}
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            isMobile ? (
                              ...
                              <div className="flex items-center gap-0.5 text-yellow-500">
                                {Array.from({ length: 5 }).map((_, i) => (
                                  <Star
                                    key={i}
                                    className={`w-4 h-4 ${i < Math.round(avg) ? "fill-yellow-500" : "fill-muted stroke-yellow-500"}`}
                                  />
                                ))}
                              </div>
                            )
                          ...
                          <Button variant="outline" size="sm" asChild>
                            <Link href={`/foodmenu/${entry.id}`}>Rate</Link>
                          </Button>
                          <Button asChild variant="ghost" size="icon">
                            <Link href={`/foodmenu/comments/${entry.id}`}>
                              <MessageSquare />
                            </Link>
                          </Button>
                        </div>
